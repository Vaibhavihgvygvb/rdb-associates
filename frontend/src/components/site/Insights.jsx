...
export default function Insights() {
  const ref = useReveal();
  return (
    <section id="insights" data-testid="insights-section" ref={ref} className="reveal relative py-24 md:py-32 bg-sage text-cream overflow-hidden">
      <div className="absolute inset-0 opacity-[0.12] bg-cover bg-center" style={{ backgroundImage: `url('${LIBRARY}')` }} aria-hidden />
      <div className="absolute inset-0 bg-gradient-to-b from-sage via-sage/95 to-sage" />
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-16">
          <div className="max-w-2xl">
            ...
            <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl leading-[1.05]">
              Notes from the <span className="italic text-brown">chambers</span>.
            </h2>
          </div>
          <Link to="/newsletter" className="inline-flex items-center gap-2 text-brown text-xs uppercase tracking-widest-plus hover:text-cream transition-colors duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {posts.map((p, i) => (
            <article key={p.title} data-testid={`insight-card-${i}`}
              className="group relative border border-brown/20 bg-sage-light/40 hover:bg-sage-light hover:border-brown/60 transition-[background-color,border-color] duration-500">
              <div className="p-8">
                <div className="flex items-center justify-between text-[10px] uppercase tracking-widest-plus text-brown/80">
                  <span>{p.tag}</span>
                  <span className="text-cream/50">{p.date}</span>
                </div>
                <h3 className="font-serif text-xl md:text-2xl text-cream mt-6 leading-snug group-hover:text-brown transition-colors duration-500">{p.title}</h3>
                <p className="text-cream/65 text-sm mt-4 leading-relaxed">{p.excerpt}</p>
                <div className="mt-8 flex items-center gap-2 text-brown text-xs uppercase tracking-widest">
